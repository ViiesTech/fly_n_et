...
const PointToPoint = ({navigation}) => {
  ...
  async function getLocations() {
    ...
    // alert('hello')
    let obj = {
      lat: parseFloat(context?.user?.latitude),
    ...
    useEffect(() => {
      const restoreSelection = async () => {
        try {
          ...
          if (defaultAirport && inputRef.current) {
            inputRef.current.setNativeProps({
  ...
  const Button = () => {
    ...
    const onSearch = async () => {
      ...
      if (
        ...
        try {
          setLoading(true);
          const locationDetails = await AsyncStorage.getItem(
            'p2p_locationDetails',
          );
          const locationDetails2 = await AsyncStorage.getItem(
            'p2p_locationDetails2',
          );
          const distance = await AsyncStorage.getItem('p2p_distance');
          const selectLocation = await AsyncStorage.getItem(
            'p2p_selectLocation',
          );
          const selectLocation2 = await AsyncStorage.getItem(
            'p2p_selectLocation2',
          );
