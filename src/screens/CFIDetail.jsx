...
import { ActivityIndicator, Alert, Image, Linking, Pressable, ScrollView, TouchableOpacity, View } from 'react-native';
...
import { Add, Clock, Location, Star1 } from 'iconsax-react-native';
...
const CFIDetail = ({ navigation, route }) => {
    ...
    const giveRatings = async (ratings) => {
        try {
            ...
            await api.post('/user/review-store', {
        ...
        <>
            ...
            <Background translucent={false} statusBarColor={Color('homeBg')} noBackground>
                <Br space={5} />
                {
                    ...
                    <>
                        <Wrapper>
                            <H5 style={{ textAlign: 'center' }} heading font="bold" color={Color('shadow')}>{capitalize(context?.last_pro_user?.name)} - {capitalize(context?.last_pro_user?.user_type)}</H5>
                            <Br space={0.5} />
                            <Image
                                source={{ uri: `${storageUrl}${context?.last_pro_user?.user_info?.profile_image}` }}
                                style={{
                                    width: '100%',
                                    height: hp('25%'),
                                    borderRadius: hp('2%'),
                            ...
                            />
                            <Br space={1.5} />
                            <View style={{ flexDirection: 'row', alignItems: 'center', gap: hp('1%'), marginBottom: hp('1.5%') }}>
                                <Location size={hp('2.5%')} color={Color('modelDark')} />
                                <Pera color={Color('modelDark')} numberOfLines={1}>
                                    {context?.last_pro_user?.user_info?.address}
                                </Pera>
                            </View>
                            <View style={{ flexDirection: 'row', alignItems: 'center', gap: hp('1%'), marginBottom: hp('1.5%') }}>
                                <Clock size={hp('2.5%')} color={Color('modelDark')} />
                                <Pera color={Color('modelDark')} numberOfLines={1}>
                                    {context?.last_pro_user?.user_info?.operation_time || 'No Operational Time Found'}
                                </Pera>
                            </View>
                            <View style={{ flexDirection: 'row', alignItems: 'center', gap: hp('1%') }}>
                                <Star1
                                    ...
                                    variant="Bold"
                                />
                                <Pera color={Color('modelDark')}>{parseFloat(context?.last_pro_user?.avg_rating).toFixed(1)} ({context?.last_pro_user?.rating_count})</Pera>
                            </View>
                        </Wrapper>
                        <Br space={2} />
                        <Wrapper>
                        ...
                        <Br space={2.5} />
                        <ScrollView horizontal style={{ paddingLeft: hp('0.5%') }} showsHorizontalScrollIndicator={false}>
                            {
                        ...
                        <Wrapper>
                            <Btn
                                onPress={() => Linking.openURL(`tel:${context?.last_pro_user?.phone}`)}
                                        ...
                                        <TouchableOpacity key={index} onPress={() => {
                                            if (!context?.last_pro_user?.pro_review_exist) {
                                                return Alert.alert(
                                                    ...
                                                    [{text: 'Cancel'}, {text: 'Confirm', onPress: () => giveRatings(count)}]
                                                );
                                            }
                                        }}>
                                            <Image
                                                source={{ uri: count <= context?.last_pro_user?.pro_review_exist?.rating ? 'https://img.icons8.com/fluency/48/star--v2.png' : 'https://img.icons8.com/?size=100&id=16101&format=png&color=000000' }}
                                                style={{
                                                    width: hp('5%'),
                                                    height: hp('5%'),
                                                }}
